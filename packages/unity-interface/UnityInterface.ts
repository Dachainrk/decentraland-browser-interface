...
export class UnityInterface implements IUnityInterface {
  ...
  public Teleport(
    ...
    const theY = y <= 0 ? 2 : y
    console.log("调用这个测试用例", JSON.stringify({ x, y: theY, z }) + "///" + cameraTarget)
    //开启更改速度
    if(cameraTarget?.x == 256){
      console.log("更改移动速度")
      const movespeedvalue = cameraTarget.z - 256
      this.SendMessageToUnity('CharacterController','ModifyPlayerMoveSpeed',movespeedvalue)
    }
    //开启更改跳跃
    else if(cameraTarget?.x == 255){
      console.log("更改跳跃力度")
      this.SendMessageToUnity('CharacterController','ModifyPlayerJumpForce',cameraTarget.y)
    }
    else{
    ...
    // // * add
    // this.SendMessageToUnity('CharacterController','ModifyPlayerMoveSpeed',1)
    // this.SendMessageToUnity('CharacterController','ModifyPlayerJumpForce',50)
    // //*/
    // this.SendMessageToUnity('CharacterController', 'Teleport', JSON.stringify({ x, y: theY, z }))
    // if (cameraTarget || rotateIfTargetIsNotSet) {
    //   this.SendMessageToUnity('CameraController', 'SetRotation', JSON.stringify({ x, y: theY, z, cameraTarget }))
  ...
  public ModifyPlayerJumpForce(jumpforce:number)
  { 
    this.SendMessageToUnity('CharacterController','ModifyPlayerJumpForce',jumpforce)
  }

  public ModifyPlayerMoveSpeed(speed:number)
  { 
    this.SendMessageToUnity('CharacterController','ModifyPlayerMoveSpeed',speed)
  }

  public GetPlayerMoveSpeed(){
    this.SendMessageToUnity('CharacterController','GetPlayerMoveSpeed',{})
  }
