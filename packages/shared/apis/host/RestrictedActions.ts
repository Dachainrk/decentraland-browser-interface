...
import type {
  ChangeRealmRequest,
  CommsAdapterRequest,
  GetPlayerMoveSpeedRequest,
  GetPlayerMoveSpeedResponse,
  ModifyPlayerMoveSpeedRequest,
  ModifyPlayerMoveSpeedResponse,
  ModifyPlayerJumpForceRequest,
  ModifyPlayerJumpForceResponse,
  MovePlayerToRequest,
...
export function getPlayerSpeed(req:GetPlayerMoveSpeedRequest,ctx:PortContext) : GetPlayerMoveSpeedResponse {
  getUnityInstance().GetPlayerMoveSpeed()
  return {}
}

export function ModifyPlayerSpeed(req:ModifyPlayerMoveSpeedRequest,ctx:PortContext) : ModifyPlayerMoveSpeedResponse{
  getUnityInstance().ModifyPlayerMoveSpeed(req.speed)
  return {}
}

export function ModifyPlayerJumpForce(req:ModifyPlayerJumpForceRequest,ctx:PortContext) : ModifyPlayerJumpForceResponse{
  getUnityInstance().ModifyPlayerMoveSpeed(req.jumpforce)
  return {}
}
...
export function movePlayerTo(req: MovePlayerToRequest, ctx: PortContext): MovePlayerToResponse {
  ...
  const test = req.cameraTarget ? basePosition.add(req.cameraTarget) : undefined
...
export function registerRestrictedActionsServiceServerImplementation(port: RpcServerPort<PortContext>) {
  codegen.registerService(port, RestrictedActionsServiceDefinition, async () => ({
    ...
    async getPlayerSpeed(req:GetPlayerMoveSpeedRequest,ctx:PortContext){
      return getPlayerSpeed(req,ctx)
    },
    async modifyPlayerSpeed(req:ModifyPlayerMoveSpeedRequest,ctx:PortContext){
        return ModifyPlayerSpeed(req,ctx)
    },
    async modifyPlayerJumpForce(req:ModifyPlayerJumpForceRequest,ctx:PortContext){
      return ModifyPlayerJumpForce(req,ctx)
  },
